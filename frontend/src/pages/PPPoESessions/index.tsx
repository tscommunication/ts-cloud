...
import { getCustomer, getCustomerNetworkPath } from '../../api/customers'
...
export default function PPPoESessions() {
  ...
  const visibleNetworkPaths = useQueries({ queries: pagedRows.filter((item) => Boolean(item.customer_id)).map((item) => ({ queryKey: ['customer-network-path', item.customer_id], queryFn: () => getCustomerNetworkPath(item.customer_id!), staleTime: 30_000 })) })
  const rxLaserByCustomerID = new Map(visibleNetworkPaths.map((query) => [query.data?.profile?.customer_id, query.data?.optical?.rx_power_dbm]))
  ...
  return <Box>
    ...
    <Grid container spacing={2} sx={{ mb: 3 }}>
      {[{ label: 'Active Users', value: activeUsers, icon: <PeopleIcon />, color: 'primary.main', mapping: 'all' }, { label: 'Mapped Users', value: mapped, icon: <VerifiedUserIcon />, color: 'success.main', mapping: 'mapped' }, { label: 'Unmapped Users', value: unmapped, icon: <LinkOffIcon />, color: unmapped > 0 ? 'warning.main' : 'text.secondary', mapping: 'unmapped' }, { label: `${usageDays}-Day RX / TX`, value: `${formatTraffic(usage.data?.rx_bytes ?? 0)} / ${formatTraffic(usage.data?.tx_bytes ?? 0)}`, icon: <PeopleIcon />, color: 'info.main' }].map((stat) => <Grid key={stat.label} size={{ xs: 12, sm: 6, md: 3 }}><Card role={stat.mapping ? 'button' : undefined} tabIndex={stat.mapping ? 0 : undefined} onClick={() => stat.mapping && setParams(stat.mapping === 'all' ? {} : { mapping: stat.mapping })} onKeyDown={(event) => { if (stat.mapping && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); setParams(stat.mapping === 'all' ? {} : { mapping: stat.mapping }) } }} sx={stat.mapping ? { cursor: 'pointer', '&:hover': { boxShadow: 4 } } : undefined}><CardContent><Box sx={{ display: 'flex', justifyContent: 'space-between' }}><Box><Typography color="text.secondary">{stat.label}</Typography><Typography variant="h5" sx={{ fontWeight: 700 }}>{stat.value}</Typography></Box><Box sx={{ color: stat.color }}>{stat.icon}</Box></Box></CardContent></Card></Grid>)}
    </Grid>
    <Card><CardContent>
      <Grid container spacing={2} sx={{ mb: 2 }}>
        <Grid size={{ xs: 12, md: 8 }}><TextField fullWidth label="Search username, customer, package, IP or router" value={search} onChange={(event) => { setSearch(event.target.value); setPage(0) }} /></Grid>
        <Grid size={{ xs: 12, md: 4 }}><TextField fullWidth select label="Mapping" value={mapping} onChange={(event) => { setPage(0); setParams(event.target.value === 'all' ? {} : { mapping: event.target.value }) }}><MenuItem value="all">All users</MenuItem><MenuItem value="mapped">Mapped only</MenuItem><MenuItem value="unmapped">Unmapped only</MenuItem></TextField></Grid>
        <Grid size={{ xs: 12 }}><FormControlLabel control={<Switch checked={showHistory} onChange={(event) => { setShowHistory(event.target.checked); setPage(0) }} />} label="Include disconnected session history" /></Grid>
      </Grid>
      <TableContainer><Table sx={{ minWidth: 1580 }} size="small"><TableHead><TableRow><TableCell>#</TableCell><TableCell>Username</TableCell><TableCell>Router</TableCell><TableCell>Customer</TableCell>{!isAgent && <TableCell>Agent</TableCell>}<TableCell>Package</TableCell><TableCell>Mapping</TableCell><TableCell>Status</TableCell><TableCell>IP Address</TableCell><TableCell>Caller ID / MAC</TableCell><TableCell>Service</TableCell><TableCell>RX Laser</TableCell><TableCell>Uptime</TableCell><TableCell>Last Seen / Disconnected</TableCell><TableCell align="right">Action</TableCell></TableRow></TableHead><TableBody>
		{pagedRows.map((item, index) => <TableRow key={item.id} hover><TableCell>{effectivePage * pageSize + index + 1}</TableCell><TableCell><Button size="small" sx={{ fontWeight: 700, textTransform: 'none' }} onClick={() => item.customer_id ? navigate(`/customers/${item.customer_id}`) : setDetailSession(item)}>{item.username}</Button></TableCell><TableCell>{item.router_code}<br />{item.router_name}</TableCell><TableCell>{item.customer_name ? <>{item.customer_code}<br />{item.customer_name}</> : '—'}</TableCell>{!isAgent && <TableCell>{item.agent_name ? <>{item.agent_code}<br />{item.agent_name}</> : 'Unassigned'}</TableCell>}<TableCell>{item.package_name ? <>{item.package_code}<br />{item.package_name}</> : '—'}</TableCell><TableCell><Chip size="small" color={item.subscription_id ? 'success' : 'warning'} label={item.subscription_id ? 'MAPPED' : 'UNMAPPED'} /></TableCell><TableCell><Chip size="small" color={item.active ? 'success' : 'default'} label={item.active ? 'ACTIVE' : 'DISCONNECTED'} />{!item.active && <Typography variant="caption" sx={{ display: 'block' }}>{item.disconnect_reason || 'UNKNOWN'}</Typography>}</TableCell><TableCell>{item.address || '—'}</TableCell><TableCell>{item.caller_id || '—'}</TableCell><TableCell>{item.service || '—'}</TableCell><TableCell>{item.customer_id && rxLaserByCustomerID.get(item.customer_id) != null ? `${rxLaserByCustomerID.get(item.customer_id)!.toFixed(2)} dBm` : '—'}</TableCell><TableCell>{item.uptime || '—'}</TableCell><TableCell>{new Date(item.last_seen_at).toLocaleString()}{item.disconnected_at && <><br /><Typography variant="caption">Disconnected: {new Date(item.disconnected_at).toLocaleString()}</Typography></>}</TableCell><TableCell align="right">{!isAgent && item.customer_id && <Button size="small" onClick={() => navigate(`/customers?search=${encodeURIComponent(item.customer_code || item.customer_id!.toString())}`)}>Manage</Button>}{item.active && <Button size="small" onClick={() => { setTrafficSession(item); setTrafficSamples([{ time: new Date().toLocaleTimeString(), rx: item.rx_rate_bps / 1_000_000, tx: item.tx_rate_bps / 1_000_000 }]) }}>Traffic</Button>}{!isAgent && !item.subscription_id && <Button size="small" onClick={() => { setMapSession(item); setSubscriptionID(null); setActionError('') }}>Map</Button>}</TableCell></TableRow>)}
		{!sessions.isLoading && rows.length === 0 && <TableRow><TableCell colSpan={isAgent ? 14 : 15} align="center">No matching PPPoE sessions.</TableCell></TableRow>}
		{sessions.isLoading && <TableRow><TableCell colSpan={isAgent ? 14 : 15} align="center">Loading PPPoE sessions...</TableCell></TableRow>}
