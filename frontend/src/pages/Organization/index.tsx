...
import { Alert, Box, Button, Card, CardContent, Chip, Dialog, DialogActions, DialogContent, DialogTitle, Grid, IconButton, MenuItem, Table, TableBody, TableCell, TableHead, TableRow, TextField, Tooltip, Typography } from '@mui/material'
...
const emptyAgent: AgentInput = { code: '', name: '', pop_id: 0, mobile: '', address: '', commission_percent: 0 }
...
export default function Organization() {
  ...
  const openAgent = (row?: Agent) => { setEditingAgent(row ?? null); setAgentForm(row ? { code: row.code, name: row.name, pop_id: row.pop_id, mobile: row.mobile, address: row.address, commission_percent: row.commission_percent } : { ...emptyAgent, pop_id: pops.find((pop) => pop.status === 'ACTIVE')?.id ?? 0 }); setAgentDialog(true) }
  ...
  return <Box>
    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3, gap: 2 }}>
      <Box><Typography variant="h4" sx={{ fontWeight: 700 }}>POP & Agents</Typography><Typography color="text.secondary">Manage the Head Office → POP → Agent distribution hierarchy.</Typography></Box>
    </Box>
    ...
    <Grid container spacing={3}>
      <Grid size={{ xs: 12, lg: 6 }}><Card><CardContent>
        ...
        <Table size="small"><TableHead><TableRow><TableCell>Code / Name</TableCell><TableCell>Manager</TableCell><TableCell>Status</TableCell><TableCell align="right">Actions</TableCell></TableRow></TableHead><TableBody>
          ...
          {!pops.length && <TableRow><TableCell colSpan={4} align="center">No POP configured.</TableCell></TableRow>}
        </TableBody></Table>
    ...
    <Dialog open={agentDialog} onClose={() => setAgentDialog(false)} fullWidth maxWidth="sm"><Box component="form" onSubmit={saveAgent}><DialogTitle>{editingAgent ? 'Edit Agent' : 'Add Agent'}</DialogTitle><DialogContent><Grid container spacing={2} sx={{ mt: 0 }}><Grid size={6}><TextField fullWidth required label="Code" disabled={!!editingAgent} value={agentForm.code} onChange={(e) => setAgentForm({ ...agentForm, code: e.target.value })} /></Grid><Grid size={6}><TextField fullWidth required label="Name" value={agentForm.name} onChange={(e) => setAgentForm({ ...agentForm, name: e.target.value })} /></Grid><Grid size={6}><TextField fullWidth required select label="POP" value={agentForm.pop_id} onChange={(e) => setAgentForm({ ...agentForm, pop_id: Number(e.target.value) })}>{pops.filter((row) => row.status === 'ACTIVE' || row.id === agentForm.pop_id).map((row) => <MenuItem key={row.id} value={row.id}>{row.code} — {row.name}</MenuItem>)}</TextField></Grid><Grid size={6}><TextField fullWidth type="number" label="Commission %" slotProps={{ htmlInput: { min: 0, max: 100, step: 0.01 } }} value={agentForm.commission_percent} onChange={(e) => setAgentForm({ ...agentForm, commission_percent: Number(e.target.value) })} /></Grid><Grid size={6}><TextField fullWidth label="Mobile" value={agentForm.mobile} onChange={(e) => setAgentForm({ ...agentForm, mobile: e.target.value })} /></Grid><Grid size={12}><TextField fullWidth multiline label="Address" value={agentForm.address} onChange={(e) => setAgentForm({ ...agentForm, address: e.target.value })} /></Grid></Grid></DialogContent><DialogActions><Button onClick={() => setAgentDialog(false)}>Cancel</Button><Button type="submit" variant="contained" disabled={saving || !agentForm.pop_id}>Save</Button></DialogActions></Box></Dialog>
  </Box>
}
