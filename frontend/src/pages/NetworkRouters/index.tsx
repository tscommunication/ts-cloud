...
import { createNetworkRouter, getNetworkRouterAlerts, getNetworkRouterHistory, getNetworkRouterPPPoESessions, getNetworkRouters, setNetworkRouterCredentials, syncNetworkRouterResource, testNetworkRouterConnection, updateNetworkRouter, type NetworkRouter, type NetworkRouterAlert, type NetworkRouterHealth, type NetworkRouterInput, type NetworkRouterPPPoESession } from '../../api/networkRouters'
...
export default function NetworkRouters() {
  ...
  const [sessionRouter, setSessionRouter] = useState<NetworkRouter | null>(null)
  const [sessions, setSessions] = useState<NetworkRouterPPPoESession[]>([])
  ...
  const openSessions = async (router: NetworkRouter) => {
    try { setSessionRouter(router); setLoadingSessions(true); setError(''); setSessions(await getNetworkRouterPPPoESessions(router.id)) }
    catch (err) { setError(getAPIErrorMessage(err, 'Failed to load active PPPoE sessions.')); setSessions([]) } finally { setLoadingSessions(false) }
  }
  ...
  return <Box sx={{ '& .MuiDialogTitle-root': { color: 'text.primary', fontWeight: 700 } }}>
    ...
    <Dialog open={sessionRouter !== null} onClose={() => setSessionRouter(null)} fullWidth maxWidth="lg"><DialogTitle>{sessionRouter?.name} — Active PPPoE Sessions ({sessions.length})</DialogTitle><DialogContent><TableContainer><Table size="small" sx={{ minWidth: 1250 }}><TableHead><TableRow><TableCell>Username</TableCell><TableCell>Customer</TableCell><TableCell>Package</TableCell><TableCell>Mapping</TableCell><TableCell>IP Address</TableCell><TableCell>Caller ID / MAC</TableCell><TableCell>Service</TableCell><TableCell>Live RX / TX</TableCell><TableCell>Uptime</TableCell><TableCell>Last Seen</TableCell></TableRow></TableHead><TableBody>{sessions.map((item) => <TableRow key={item.id}><TableCell><b>{item.username}</b></TableCell><TableCell>{item.customer_name ? <>{item.customer_code}<br />{item.customer_name}</> : '—'}</TableCell><TableCell>{item.package_name ? <>{item.package_code}<br />{item.package_name}</> : '—'}</TableCell><TableCell><Chip size="small" color={item.subscription_id ? 'success' : 'warning'} label={item.subscription_id ? 'MAPPED' : 'UNMAPPED'} /></TableCell><TableCell>{item.address || '—'}</TableCell><TableCell>{item.caller_id || '—'}</TableCell><TableCell>{item.service || '—'}</TableCell><TableCell>↓ {formatRate(item.rx_rate_bps)}<br />↑ {formatRate(item.tx_rate_bps)}</TableCell><TableCell>{item.uptime || '—'}</TableCell><TableCell>{new Date(item.last_seen_at).toLocaleString()}</TableCell></TableRow>)}{!loadingSessions && sessions.length === 0 && <TableRow><TableCell colSpan={10} align="center">No active PPPoE sessions.</TableCell></TableRow>}{loadingSessions && <TableRow><TableCell colSpan={10} align="center">Loading active sessions...</TableCell></TableRow>}</TableBody></Table></TableContainer></DialogContent><DialogActions><Button onClick={() => setSessionRouter(null)}>Close</Button></DialogActions></Dialog>
  </Box>
}
