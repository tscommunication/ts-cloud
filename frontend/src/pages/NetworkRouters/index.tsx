...
export default function NetworkRouters() {
  ...
  return <Box sx={{ '& .MuiDialogTitle-root': { color: 'text.primary', fontWeight: 700 } }}>
    ...
    <Card sx={{ mt: 3 }}><CardContent><Typography variant="h6" sx={{ fontWeight: 700, mb: 2 }}>Active Resource Alerts</Typography><TableContainer><Table size="small"><TableHead><TableRow><TableCell>Router</TableCell><TableCell>Type</TableCell><TableCell>Current</TableCell><TableCell>Threshold</TableCell><TableCell>Opened</TableCell><TableCell>Message</TableCell></TableRow></TableHead><TableBody>{alerts.map((alert) => <TableRow key={alert.id}><TableCell>{alert.router_code} — {alert.router_name}</TableCell><TableCell><Chip size="small" color="warning" label={alert.type.replace('HIGH_', 'HIGH ')} /></TableCell><TableCell>{alert.current_value.toFixed(1)}%</TableCell><TableCell>{alert.threshold.toFixed(1)}%</TableCell><TableCell>{new Date(alert.opened_at).toLocaleString()}</TableCell><TableCell>{alert.message}</TableCell></TableRow>)}{alerts.length === 0 && <TableRow><TableCell colSpan={6} align="center">No active CPU or memory alerts.</TableCell></TableRow>}</TableBody></Table></TableContainer></CardContent></Card>
