...
export interface Agent { id: number; code: string; name: string; pop_id: number; pop_name: string; mobile: string; address: string; commission_percent: number; status: 'ACTIVE' | 'INACTIVE' }
export type POPInput = Omit<POP, 'id' | 'status'>
export type AgentInput = Omit<Agent, 'id' | 'status' | 'pop_name'>
