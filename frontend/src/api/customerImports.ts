...
export interface CustomerImportPreview { total_rows: number; active_rows: number; inactive_rows: number; packages: string[]; pops: string[]; warnings: string[] }
export interface CustomerImportBatch { id: number; filename: string; status: string; total_rows: number; imported_rows: number; created_packages: number; created_pops: number; created_at: string }
