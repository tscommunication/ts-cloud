...
func VoidPayment(
	...
	err := database.DB.Transaction(func(tx *gorm.DB) error {
		...
		if reversalResult.Reversed {
			if err := reconcileSubscriptionPaymentMetadataAfterVoidTx(
				tx,
				reversalResult.SubscriptionID,
				invoice.DueAmount,
			); err != nil {
				return err
			}
...
func reconcileSubscriptionPaymentMetadataAfterVoidTx(
	tx *gorm.DB,
	subscriptionID uint,
	invoiceDueAmount float64,
) error {
	if tx == nil {
	...
	case err == nil:
		...
		subscription.LastPaidAmount = latest.Amount

	case errors.Is(err, gorm.ErrRecordNotFound):
		subscription.LastPaymentDate = nil
		subscription.LastPaidAmount = 0

	default:
		return err
	}

	// The reversed renewal was generated by the invoice being voided.
	// After financial reconciliation, that invoice's due amount is the
	// authoritative outstanding amount for this reversal transition.
	subscription.DueAmount = invoiceDueAmount
