...
func getAgentDashboardSummary(db *gorm.DB, agentID uint, now time.Time) (*AgentDashboardSummary, error) {
	...
	if err := customers.Count(&summary.TotalCustomers).Error; err != nil { return nil, err }
	if err := customers.Where("status = ?", "ACTIVE").Count(&summary.ActiveCustomers).Error; err != nil { return nil, err }
	...
	if err := subscriptions.Count(&summary.ActiveSubscriptions).Error; err != nil { return nil, err }

	invoices := db.Model(&models.Invoice{}).
		Joins("JOIN customers ON customers.id = invoices.customer_id").
		Where("customers.agent_id = ? AND invoices.status <> ?", agentID, "CANCELLED")
	if err := invoices.Select("COALESCE(SUM(invoices.total_amount), 0)").Scan(&summary.TotalInvoiced).Error; err != nil { return nil, err }
	if err := invoices.Select("COALESCE(SUM(invoices.due_amount), 0)").Scan(&summary.TotalOutstanding).Error; err != nil { return nil, err }
	if err := invoices.Where("invoices.status = ?", "OVERDUE").Count(&summary.OverdueInvoices).Error; err != nil { return nil, err }

	collections := db.Model(&models.AgentCollection{}).Where("agent_id = ? AND status = ?", agentID, "ACTIVE")
	if err := collections.Select("COALESCE(SUM(amount), 0)").Scan(&summary.TotalCollected).Error; err != nil { return nil, err }
	dayStart := time.Date(now.Year(), now.Month(), now.Day(), 0, 0, 0, 0, now.Location())
	if err := collections.Where("collected_at >= ?", dayStart).Select("COALESCE(SUM(amount), 0)").Scan(&summary.TodayCollected).Error; err != nil { return nil, err }
