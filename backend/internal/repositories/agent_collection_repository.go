...
func ListAgentCollections(agentID uint, status string) ([]models.AgentCollection, AgentCollectionSummary, error) {
	...
	if err := query.Select("COUNT(*) AS count, COALESCE(SUM(amount),0) AS total_amount, COALESCE(SUM(commission_amount),0) AS total_commission").Scan(&summary).Error; err != nil {
		return nil, AgentCollectionSummary{}, err
	}
